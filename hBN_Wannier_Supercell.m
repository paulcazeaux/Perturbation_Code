...
L = Lattice([2.1651  2.1651;  -1.25  1.25]);
patch = Patch(L, 6);
...
% Deduce supercell blocks from the perturbation formula
fprintf('==============\nComputing the relevant supercell blocks...')
O = [0;0];
...
fprintf('==============\nConstructing superlattice matrices...')
...
super_w = squeeze(sum(S.Change_W(basis, patch, V.K_list, D1.W),3));
fprintf('   Done\n')
...
N = 10;
t = [0 1e4.^((0:N-1)/(N-1)-1)*1e3];

err0 = zeros(1,N+1); err1 = zeros(1,N+1);
parfor i = 1:N+1
    D = PeriodicDensityMatrix(S, super_basis, super_patch, super_h+t(i)*super_w, super_s, S.N*gap_position, Nq);
    B = D.Block(O,O);
    err1(i) = max(err1(i), norm(B - B0 - t(i)*B1, Inf));
    err0(i) = max(err0(i), norm(B - B0, Inf));
end
...
loglog(t, err1, 'r+-');
xlabel('Artificial scaling of the external potential', 'Interpreter', 'latex');
ylabel({'$L^2$ error'}, 'Interpreter', 'latex')
legend({'$D-D_0$', '$D-D_0-D_1$'}, 'Location', 'Northwest', 'Interpreter', 'latex')
title('\textbf{Error plot as a function of perturbation intensity using Wannier functions}', 'Interpreter', 'latex')
%%
savefig('ErrorPlot_Wannier_PerturbationIntensity.fig')
saveas(gcf, 'ErrorPlot_Wannier_PerturbationIntensity' , 'epsc')
